...
target_points = [ 1, 1, 0;
                 ...
                 -1,-1, 0;
                  1,-1, 0];
Tg = S.Tend;     
target_points = TransformPoints(Tg*txyz(0,0,5),target_points);
...
%     JT = Jp * c_J_g * S.W * S.Jacobian; %Task_Jacobian
%     %inv_Task_Jacobian = (JT'*JT + eye(length(S.q))^2)\JT';
%     inv_Task_Jacobian = dampedLeastSquaresInverse(JT,S.q,S.qL,S.qU);
%     dq = Lambda * inv_Task_Jacobian * error;
%     S = S.update(dq);
%     %S.q = S.q + dq;
%     S.Tend = S.FK;
%     W_T_c = S.Tend*Tgc;
